...
public class LLRPBitList {
    ...
    /**
         * bytes Integererpreted in order they appear in array
         * @param bytes
         */
    ...
    public void append(LLRPBitList other) {
        ...
        for (Integer i = 0; i < other.length; i++) {
            ...
            // the used BiSet adjust the length of the underlying BitSet automatically
            if (other.bits.get(i)) {
    ...
    public LLRPBitList subList(Integer from, Integer subLength)
        throws LLRPException {
        if (from < 0) {
            //			logger.error("try to start sublist at negative position - this is not possible");
            throw new LLRPException(
                "try to start sublist at negative position - this is not possible");
        }

        if (length < (from + subLength)) {
            //			logger.error("list not long enough. List has "+length+" elements, tried to get sublist from "+from+" with length "+subLength);
