...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:choose>
          ...
          <xsl:when test="@type = 's8v'">SignedByteArray</xsl:when>
    ...
    <xsl:choose>
      <xsl:when test="@enumeration and @type!='u8v'">
        private Int16 <xsl:value-of select="@name"/>_len = <xsl:choose>
          ...
          <xsl:when test="@type = 's6'">16;</xsl:when>
  ...
  <xsl:template name="DefineKnownCustomParams">
    <xsl:for-each select="../llrp:parameter">
      <xsl:if test="@type!='Custom'">"<xsl:call-template name='DefineParameterName'/>", </xsl:if>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template name="VendorPARAMDecodeFromBitArray">
    ...
    <xsl:for-each select="*">
      <xsl:if test="name()='field'">
        ...
        <xsl:if test="@type='u1v' or @type='u8v' or @type='s8v' or @type='u16v' or @type='s16v' or @type='u32v' or @type='s32v' or @type='utf8v' or @type='bytesToEnd'">
          ...
          <xsl:if test='@name=$choiceParameterName'>
            <xsl:for-each select='*'>
              <xsl:choose>
                ...
                {
                    if (obj.<xsl:copy-of select='$choiceParameterName'/>.AddCustomParameter(_param_Custom))
                    {
  ...
  <xsl:template name="VendorMSGToString">
        ...
        {
            int len;
            string xml_str = "<xsl:text disable-output-escaping="yes">&lt;</xsl:text>" + LLRPExtensionConstants.NAMESPACE_PREFIX + ":" + "<xsl:value-of select="@name"/>";
    ...
    <xsl:for-each select="*">
      <xsl:choose>
        <xsl:when test="name()='field'">
          ...
          <xsl:choose>
            ...
            <xsl:when test="@type='u1v'">
                    xml_str += "  <xsl:text disable-output-escaping="yes">&lt;</xsl:text>" + LLRPExtensionConstants.NAMESPACE_PREFIX + ":" + "<xsl:value-of select="@name"/> Count=\"" + <xsl:value-of select="@name"/>.Count + "\"<xsl:text disable-output-escaping="yes">&gt;</xsl:text>" + Util.ConvertArrayTypeToString(<xsl:value-of select="@name"/>, "<xsl:value-of select="@type"/>", "<xsl:value-of select="@format"/>") + "<xsl:text disable-output-escaping="yes">&lt;/</xsl:text>" + LLRPExtensionConstants.NAMESPACE_PREFIX + ":" + "<xsl:value-of select="@name"/><xsl:text disable-output-escaping="yes">&gt;</xsl:text>";
            </xsl:when>
        ...
        <xsl:otherwise>
            ...
            xml_str += "<xsl:text disable-output-escaping="yes">&lt;/</xsl:text>" + LLRPExtensionConstants.NAMESPACE_PREFIX + ":" + "<xsl:value-of select="@name"/><xsl:text disable-output-escaping="yes">&gt;</xsl:text>";
            return xml_str;
        }
  </xsl:template>
    ...
    <xsl:for-each select="*">
      <xsl:choose>
        ...
        <xsl:when test="name()='parameter'">
          ...
          <xsl:choose>
            <xsl:when test="@type='Custom'">
            string[] excl = new string[] { <xsl:call-template name='DefineKnownCustomParams'/> };
          ...
          <xsl:for-each select='../../llrp:customChoiceDefinition'>
            ...
            try
            {
                foreach (XmlNode ccnode in node.ChildNodes)
                {
                    ...
                    {
              <xsl:for-each select='*'>
                    case LLRPExtensionConstants.NAMESPACE_PREFIX + ":<xsl:call-template name='DefineParameterName'/>":
                <xsl:choose>
                  <xsl:when test="@type='Custom'">
                    default:
                        ICustom_Parameter custom =
  ...
  <xsl:template name ="VendorMSGDecodeFromBitArray">
        /// <xsl:text disable-output-escaping="yes">&lt;</xsl:text>summary<xsl:text disable-output-escaping="yes">&gt;</xsl:text>
  ...
  <xsl:template name ="VendorMSGEncodeToBitArray">
        /// <xsl:text disable-output-escaping="yes">&lt;</xsl:text>summary<xsl:text disable-output-escaping="yes">&gt;</xsl:text>
  ...
  <xsl:template name ="VendorMSGToCustomMessage">
        /// <xsl:text disable-output-escaping="yes">&lt;</xsl:text>summary<xsl:text disable-output-escaping="yes">&gt;</xsl:text>
  ...
  <xsl:template name ="VendorMSGFromCustomMessage">
        /// <xsl:text disable-output-escaping="yes">&lt;</xsl:text>summary<xsl:text disable-output-escaping="yes">&gt;</xsl:text>
        /// Parse MSG_CUSTOM_MESSAGE to vendor extended message (deprecated)
        /// <xsl:text disable-output-escaping="yes">&lt;</xsl:text>/summary<xsl:text disable-output-escaping="yes">&gt;</xsl:text>
        /// <xsl:text disable-output-escaping="yes">&lt;</xsl:text>param name="msg"<xsl:text disable-output-escaping="yes">&gt;</xsl:text>Custom message to be parsed<xsl:text disable-output-escaping="yes">&lt;</xsl:text>/param<xsl:text disable-output-escaping="yes">&gt;</xsl:text>
        /// <xsl:text disable-output-escaping="yes">&lt;</xsl:text>returns<xsl:text disable-output-escaping="yes">&gt;</xsl:text>Vendor extended message<xsl:text disable-output-escaping="yes">&lt;</xsl:text>/returns<xsl:text disable-output-escaping="yes">&gt;</xsl:text>
        [Obsolete("Custom messages are derived from MSG_CUSTOM_MESSAGE")]
        public static MSG_<xsl:value-of select="@name"/> FromCustomMessage(MSG_CUSTOM_MESSAGE msg)
        {
            return msg as MSG_<xsl:value-of select="@name"/>;
        }
  </xsl:template>
</xsl:stylesheet>
